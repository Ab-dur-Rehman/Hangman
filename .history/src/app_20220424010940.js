...
const gameKeypad = {
    ...
    generateKeyPad() {
        ...
        return this.letters.map(
            (letter) => <button key={key_counter++} type="button" onClick={matchWord(this.value)} value={letter}>{letter}</button>
        );
    },    
}
const game_keypad = (
...
function matchWord(clicked_value) {
    let clickedWord = clicked_value;
