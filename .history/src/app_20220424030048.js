    ...
    const setGameWord = (word) => {
        ...
        wordArray.map (
            (letter) => {
                ...
                letter_box.innerHTML = '<span class="ind_letter_'+ +'">'+letter+'</span>';
...
const gameKeypad = {
    ...
    matchWord (el) {
        ...
        if (letter_index.length > 0) {
            letter_index.map(
                (index) => {
                    let letter_box = document.getElementsByClassName("box_"+index)[0];
            ...
            let hm_stand = document.getElementsByClassName("hm_stand")[0];
            let hm_hook = document.getElementsByClassName("hm_hook")[0];
            switch (hm_body.style.display) {
                case "none":
